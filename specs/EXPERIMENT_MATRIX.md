...
| future separately keyed aliased safe-chain | feasible CEGAR discovery of an omitted geometric distinction |
...
quotient planning. The third is not an executable V0 row until separately frozen.
...
Unregistered reward extrapolation, post-hoc test-derived predicates, statistical substitution for exact Gate rows, causal component attribution from a single ladder order, and automatic-symmetry-discovery claims from the supplied `D4` profile.

## Known failure modes

Seed selection bias, survivorship bias from dropped failures, incomparable J-level inputs, clock noise, counting fallback outside end-to-end cost, pooling distinct coverage-specific builds as one reusable model, and mixing known-symmetry positive controls into refinement-performance aggregates.

## Open risks

The canonical G2048 primary and sensitivity rows remain `INFEASIBLE_QUERY`: exact first-step failure is `383/410`, above the maximum registered `delta=0.10`. That row tests sound risk audit and charged fallback but cannot demonstrate nontrivial constrained planning. The separately keyed safe-chain row now has a fully frozen exact `D4` profile; the remaining risk is implementation/certificate conformance, not semantic ambiguity. A feasible CEGAR positive control remains future separately keyed work.
