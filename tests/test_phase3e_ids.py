from fractions import Fraction
import hashlib

import pytest

from acfqp.artifacts import object_id
from acfqp import phase3e_ids as ids
from acfqp.phase3e_ids import (
    ABSTRACT_ONLY_OCCURRENCE_WORK_SUM_DOMAIN,
    ABSTRACT_PLAN_AUDIT_DOMAIN,
    CARDINALITY_EVIDENCE_DOMAIN,
    CAUSAL_EVIDENCE_DOMAIN,
    COMPARISON_PROFILE_DOMAIN,
    COUNTER_REGISTRY_DOMAIN,
    DECISION_POINT_DOMAIN,
    DEPENDENT_FRONTIER_DERIVATION_DOMAIN,
    DEPENDENT_POSTAUDIT_OBLIGATION_DOMAIN,
    DEPENDENT_TRANSACTION_BENCHMARK_PROFILE_DOMAIN,
    FRONTIER_SNAPSHOT_DOMAIN,
    EXACT_CACHED_INFEASIBILITY_PROOF_DOMAIN,
    EXACT_KERNEL_CONTEXT_IDENTITY_DOMAIN,
    GROUND_BINDING_AFTER_FAILED_AUDIT_DOMAIN,
    MAX_CANONICAL_INTEGER_DECIMAL_DIGITS,
    MODEL_ONLY_ORCHESTRATION_BINDING_DOMAIN,
    MODEL_ONLY_OPERATIONAL_EXECUTION_DOMAIN,
    MODEL_ONLY_OPERATIONAL_REQUEST_DOMAIN,
    MODEL_ONLY_RESULT_DOMAIN,
    MODEL_FAILURE_OCCURRENCE_CLOSURE_DOMAIN,
    OCCURRENCE_CLOSURE_EVIDENCE_DOMAIN,
    PHASE3E_DOMAIN_TAGS,
    PLAN_FROZEN_EXACT_CACHE_BINDING_DOMAIN,
    PORTABLE_POLICY_BINDING_DOMAIN,
    PORTABLE_SOUND_BELLMAN_PROOF_DOMAIN,
    PRESELECTION_NOT_APPLICABLE_BINDING_DOMAIN,
    RAPM_SOURCE_LEASE_DOMAIN,
    ROUTE_CAP_PROFILE_DOMAIN,
    ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN,
    SELECTED_CONTINGENT_PLAN_DOMAIN,
    TRANSACTION_DOMAIN,
    VERIFIED_EXACT_INFEASIBILITY_SOURCE_DOMAIN,
    Phase3EIdentityError,
    canonical_json,
    canonical_json_bytes,
    content_id,
    loads_canonical_json,
    parse_content_id,
    require_exact_fields,
    verify_content_id,
)


def test_registry_contains_all_normative_fq3_domains() -> None:
    assert {
        "acfqp:route-upper-bound-envelope:v1",
        "acfqp:comparison-profile:v1",
        "acfqp:counter-registry:v1",
        "acfqp:cardinality-evidence:v1",
        "acfqp:route-cap-profile:v1",
        "acfqp:frontier-snapshot:v1",
        "acfqp:causal-evidence:v1",
        "acfqp:decision-point:v1",
        "acfqp:transaction:v1",
    }.issubset(PHASE3E_DOMAIN_TAGS)
    assert {
        ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN,
        COMPARISON_PROFILE_DOMAIN,
        COUNTER_REGISTRY_DOMAIN,
        CARDINALITY_EVIDENCE_DOMAIN,
        ROUTE_CAP_PROFILE_DOMAIN,
        FRONTIER_SNAPSHOT_DOMAIN,
        CAUSAL_EVIDENCE_DOMAIN,
        DECISION_POINT_DOMAIN,
        TRANSACTION_DOMAIN,
    }.issubset(PHASE3E_DOMAIN_TAGS)


def test_registry_includes_routing_and_accounting_v1_domains() -> None:
    assert {
        "acfqp:route-decision-context:v1",
        "acfqp:route-decision:v1",
        "acfqp:trusted-budget-replay:v1",
        "acfqp:terminal-artifact:v1",
        "acfqp:typed-verification-attestation:v1",
        "acfqp:counter-record:v1",
        "acfqp:work-vector:v1",
        "acfqp:comparison-vector:v1",
        "acfqp:native-zero-attestation:v1",
        "acfqp:reconciliation-proof:v1",
        "acfqp:actual-projection-profile:v1",
        "acfqp:actual-projection-proof:v1",
        "acfqp:occurrence-work-sum:v1",
        "acfqp:workload-vector-spec:v1",
        "acfqp:workload-vector-prefix:v1",
        "acfqp:workload-vector-analysis:v1",
        "acfqp:logical-occurrence:v1",
        "acfqp:route-attempt:v1",
        "acfqp:rebuild-policy:v1",
        "acfqp:rebuild-event:v1",
        "acfqp:campaign-occurrence-closure:v1",
        "acfqp:campaign-summary:v1",
        PRESELECTION_NOT_APPLICABLE_BINDING_DOMAIN,
    }.issubset(PHASE3E_DOMAIN_TAGS)


def test_registry_includes_model_only_audit_and_exact_cache_domains() -> None:
    assert {
        RAPM_SOURCE_LEASE_DOMAIN,
        SELECTED_CONTINGENT_PLAN_DOMAIN,
        PORTABLE_POLICY_BINDING_DOMAIN,
        PORTABLE_SOUND_BELLMAN_PROOF_DOMAIN,
        ABSTRACT_PLAN_AUDIT_DOMAIN,
        PLAN_FROZEN_EXACT_CACHE_BINDING_DOMAIN,
        VERIFIED_EXACT_INFEASIBILITY_SOURCE_DOMAIN,
        EXACT_CACHED_INFEASIBILITY_PROOF_DOMAIN,
        EXACT_KERNEL_CONTEXT_IDENTITY_DOMAIN,
        MODEL_ONLY_ORCHESTRATION_BINDING_DOMAIN,
        MODEL_ONLY_RESULT_DOMAIN,
        ABSTRACT_ONLY_OCCURRENCE_WORK_SUM_DOMAIN,
        MODEL_ONLY_OPERATIONAL_REQUEST_DOMAIN,
        MODEL_ONLY_OPERATIONAL_EXECUTION_DOMAIN,
        GROUND_BINDING_AFTER_FAILED_AUDIT_DOMAIN,
    }.issubset(PHASE3E_DOMAIN_TAGS)

    payload = {"schema": "same-payload"}
    identifiers = {
        content_id(domain, payload)
        for domain in (
            RAPM_SOURCE_LEASE_DOMAIN,
            SELECTED_CONTINGENT_PLAN_DOMAIN,
            PORTABLE_SOUND_BELLMAN_PROOF_DOMAIN,
            ABSTRACT_PLAN_AUDIT_DOMAIN,
            EXACT_CACHED_INFEASIBILITY_PROOF_DOMAIN,
            EXACT_KERNEL_CONTEXT_IDENTITY_DOMAIN,
            MODEL_ONLY_RESULT_DOMAIN,
            ABSTRACT_ONLY_OCCURRENCE_WORK_SUM_DOMAIN,
            MODEL_ONLY_OPERATIONAL_REQUEST_DOMAIN,
            MODEL_ONLY_OPERATIONAL_EXECUTION_DOMAIN,
            GROUND_BINDING_AFTER_FAILED_AUDIT_DOMAIN,
        )
    }
    assert len(identifiers) == 11


def test_registry_role_separates_dependent_transaction_domains() -> None:
    domains = (
        DEPENDENT_POSTAUDIT_OBLIGATION_DOMAIN,
        DEPENDENT_FRONTIER_DERIVATION_DOMAIN,
        DEPENDENT_TRANSACTION_BENCHMARK_PROFILE_DOMAIN,
    )
    assert set(domains).issubset(PHASE3E_DOMAIN_TAGS)
    payload = {"schema": "same-dependent-transaction-payload"}
    assert len({content_id(domain, payload) for domain in domains}) == 3


def test_model_failure_occurrence_closure_has_its_own_domain_role() -> None:
    assert MODEL_FAILURE_OCCURRENCE_CLOSURE_DOMAIN in PHASE3E_DOMAIN_TAGS
    assert (
        MODEL_FAILURE_OCCURRENCE_CLOSURE_DOMAIN
        != OCCURRENCE_CLOSURE_EVIDENCE_DOMAIN
    )
    payload = {"schema": "same-occurrence-closure-payload"}
    assert content_id(
        MODEL_FAILURE_OCCURRENCE_CLOSURE_DOMAIN, payload
    ) != content_id(OCCURRENCE_CLOSURE_EVIDENCE_DOMAIN, payload)


def test_canonical_json_is_sorted_compact_utf8_and_reduces_fractions() -> None:
    left = {
        "中文": "商空间",
        "probability": Fraction(2, 6),
        "nested": [{"z": 2, "a": 1}],
    }
    right = {
        "nested": [{"a": 1, "z": 2}],
        "probability": Fraction(1, 3),
        "中文": "商空间",
    }

    expected = (
        '{"nested":[{"a":1,"z":2}],"probability":'
        '{"denominator":3,"numerator":1},"中文":"商空间"}'
    )
    assert canonical_json(left) == expected
    assert canonical_json_bytes(left) == expected.encode("utf-8")
    assert canonical_json_bytes(left) == canonical_json_bytes(right)
    assert loads_canonical_json(expected.encode("utf-8")) == right


def test_canonical_json_round_trips_registered_huge_exact_integers() -> None:
    huge = 10**5_000 + 7
    document = {"huge": huge, "negative": -huge}

    encoded = canonical_json_bytes(document)

    assert encoded.startswith(b'{"huge":1')
    assert loads_canonical_json(encoded) == document
    assert canonical_json_bytes(loads_canonical_json(encoded)) == encoded


def test_canonical_json_huge_integer_support_remains_locally_bounded() -> None:
    oversized = b"1" + (
        b"0" * MAX_CANONICAL_INTEGER_DECIMAL_DIGITS
    )

    with pytest.raises(Phase3EIdentityError, match="digit ceiling"):
        loads_canonical_json(oversized)
    with pytest.raises(Phase3EIdentityError, match="digit ceiling"):
        canonical_json_bytes(
            {"oversized": 10**MAX_CANONICAL_INTEGER_DECIMAL_DIGITS}
        )


def test_content_id_is_the_normative_full_domain_separated_sha256() -> None:
    document = {"query_id": "query-1", "limit": Fraction(1, 20)}
    expected = hashlib.sha256(
        ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN.encode("utf-8")
        + b"\x00"
        + canonical_json_bytes(document)
    ).hexdigest()

    identifier = content_id(ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN, document)
    assert identifier == expected
    assert len(identifier) == 64
    assert identifier == identifier.lower()
    assert parse_content_id(identifier) == identifier
    assert verify_content_id(
        ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN, document, identifier
    )


def test_registry_includes_causal_promotion_terminal_and_bundle_verifier_domains() -> None:
    domains = {
        ids.V075_K7_CAUSAL_PROMOTION_BUDGET_REPLAY_ATTESTATION_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_ROUTE_CONTEXT_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_ROUTE_ATTEMPT_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_TERMINAL_DERIVATION_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_COMPLETE_BUNDLE_VERIFICATION_PROFILE_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_COMPLETE_BUNDLE_SEMANTIC_VERIFIER_V1_DOMAIN,
        ids.V075_K7_CAUSAL_PROMOTION_COMPLETE_BUNDLE_VERIFICATION_V1_DOMAIN,
    }
    assert domains.issubset(ids.PHASE3E_DOMAIN_TAGS)
    payload = {"schema": "same-causal-promotion-terminal-payload"}
    assert len({ids.content_id(domain, payload) for domain in domains}) == len(
        domains
    )


def test_equal_json_in_different_domains_cannot_reuse_an_id() -> None:
    document = {"schema_version": "1.0.0", "payload": []}
    upper_id = content_id(ROUTE_UPPER_BOUND_ENVELOPE_DOMAIN, document)
    decision_id = content_id(DECISION_POINT_DOMAIN, document)

    assert upper_id != decision_id
    assert not verify_content_id(DECISION_POINT_DOMAIN, document, upper_id)


@pytest.mark.parametrize(
    "bad_value",
    (
        float("nan"),
        float("inf"),
        float("-inf"),
        {"nested": float("nan")},
    ),
)
def test_nonfinite_numbers_are_rejected(bad_value: object) -> None:
    with pytest.raises(Phase3EIdentityError, match="non-finite"):
        canonical_json_bytes(bad_value)


@pytest.mark.parametrize(
    "bad_value",
    (
        (1, 2),
        {1, 2},
        b"bytes",
        {1: "non-string-key"},
        object(),
    ),
)
def test_unsupported_python_values_are_rejected(bad_value: object) -> None:
    with pytest.raises(Phase3EIdentityError):
        canonical_json_bytes(bad_value)


@pytest.mark.parametrize(
    "bad_value",
    (
        {"numerator": 2, "denominator": 4},
        {"numerator": 1, "denominator": -3},
        {"numerator": True, "denominator": 3},
        {"numerator": 1},
        {"numerator": 1, "denominator": 3, "extra": 0},
    ),
)
def test_noncanonical_rational_records_are_rejected(bad_value: object) -> None:
    with pytest.raises(Phase3EIdentityError, match="rational"):
        canonical_json_bytes(bad_value)


@pytest.mark.parametrize(
    "bad_json",
    (
        '{"b":2,"a":1}',
        '{"a":1, "b":2}',
        '{"a":1}\n',
        '{"a":1,"a":1}',
        '{"a":NaN}',
        '{"a":1e0}',
        '{"denominator":4,"numerator":2}',
    ),
)
def test_strict_loader_rejects_noncanonical_or_unsafe_json(bad_json: str) -> None:
    with pytest.raises(Phase3EIdentityError):
        loads_canonical_json(bad_json)


def test_strict_loader_rejects_non_utf8_bytes() -> None:
    with pytest.raises(Phase3EIdentityError, match="UTF-8"):
        loads_canonical_json(b'"\xff"')


def test_unregistered_domains_and_noncanonical_ids_are_rejected() -> None:
    with pytest.raises(Phase3EIdentityError, match="unregistered"):
        content_id("acfqp:unregistered:v1", {"a": 1})
    for invalid in ("0" * 63, "0" * 65, "A" * 64, "obj-" + "0" * 64, 7):
        with pytest.raises(Phase3EIdentityError, match="64 lowercase"):
            parse_content_id(invalid)  # type: ignore[arg-type]


def test_exact_field_helper_rejects_missing_extra_and_non_string_fields() -> None:
    require_exact_fields({"a": 1, "b": 2}, ("a", "b"), context="fixture")

    with pytest.raises(
        Phase3EIdentityError,
        match=r"missing=\['b'\].*unexpected=\['c'\]",
    ):
        require_exact_fields({"a": 1, "c": 3}, ("a", "b"), context="fixture")
    with pytest.raises(Phase3EIdentityError, match="field names must be strings"):
        require_exact_fields({1: "bad"}, ("a",), context="fixture")  # type: ignore[dict-item]


def test_cycles_are_rejected_instead_of_recursing_forever() -> None:
    cyclic: list[object] = []
    cyclic.append(cyclic)
    with pytest.raises(Phase3EIdentityError, match="cyclic"):
        canonical_json_bytes(cyclic)


def test_phase3e_module_does_not_change_legacy_object_ids() -> None:
    assert object_id(
        {"probability": Fraction(1, 3), "states": [1, 2, 3]}
    ) == "obj-79957ed5a3639f5e"


def test_k7_complete_accounting_successor_domains_are_central_and_role_separated() -> None:
    domains = (
        ids.CONSTRUCTION_K7_ROUTE_TERMINAL_SEMANTIC_DEPENDENCY_V2_DOMAIN,
        ids.CONSTRUCTION_K7_EXACT_ROUTE_DERIVED_PATH_PROOF_V2_DOMAIN,
        ids.CONSTRUCTION_K7_COMPLETE_DERIVED_RECONCILIATION_READINESS_V2_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_SOURCE_HOOK_INVENTORY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_OWNER_WINDOW_CLOSURE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_STAGE_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_BRANCH_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_REPLACEMENT_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_NATIVE_ZERO_SEMANTIC_VERIFIER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROFILE_NATIVE_ZERO_ATTESTATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROFILE_NATIVE_ZERO_ENVELOPE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_SEMANTIC_PATH_RECORDER_AUTHORITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_SEMANTIC_PATH_RESOLUTION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_SEMANTIC_EVIDENCE_CLOSURE_CONTEXT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_SEMANTIC_EVIDENCE_CLOSURE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_FORMAL_ACTUAL_PROJECTION_PROOF_V6_DOMAIN,
        ids.CONSTRUCTION_K7_FORMAL_ACCOUNTING_MATERIALIZATION_BUNDLE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ROOT_CAP_SEMANTICS_PROFILE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ROOT_CAP_EXHAUSTION_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ROOT_CAP_ATTEMPT_TERMINAL_AUTHORITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ROOT_CAP_TERMINAL_ACCOUNTING_BUNDLE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ROOT_CAP_TERMINAL_ACCOUNTING_VERIFICATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PRODUCTION_COMPLETE_BUNDLE_VERIFICATION_PROFILE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PRODUCTION_COMPLETE_BUNDLE_SEMANTIC_VERIFIER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PRODUCTION_COMPLETE_BUNDLE_EVALUATION_RECORDER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PRODUCTION_COMPLETE_BUNDLE_VERIFICATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_LOGICAL_OCCURRENCE_WORK_SUM_V1_DOMAIN,
        ids.CONSTRUCTION_K7_LOGICAL_OCCURRENCE_CLOSURE_AUTHORITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_LOGICAL_OCCURRENCE_CLOSURE_BUNDLE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_LOGICAL_OCCURRENCE_CLOSURE_VERIFICATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_ACCOUNTING_PROFILE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_ACCOUNTING_PROFILE_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_V075_TERMINAL_STATUS_INVENTORY_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_DURABLE_PROOF_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_IDENTITY_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_STRUCTURAL_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_QUERY_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_KERNEL_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_BUILD_EPOCH_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_THRESHOLD_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_REWARD_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_POLICY_CLASS_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_SEARCH_PROFILE_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_STATE_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_STATE_ACTION_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_SOURCE_PROJECTION_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_VERIFICATION_PROFILE_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_VERIFICATION_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_CACHE_CONSUMPTION_V1_DOMAIN,
        ids.PHASE3E_EXACT_INFEASIBILITY_BLOCKER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_EXPECTED_ARTIFACT_IDENTITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_ATTEMPT_CONTEXT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_ACCESS_EVENT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_READ_RECEIPT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_ACCESS_SEQUENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_PREFIX_RECORDER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_PREFIX_COMPLETENESS_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_TERMINAL_AUTHORITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_FAILURE_BUNDLE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_INTEGRITY_FAILURE_VERIFICATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_OPERATION_BOUNDARY_MANIFEST_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_OPERATION_BOUNDARY_SOURCE_ARCHIVE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_OPERATION_BOUNDARY_SITE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_OPERATION_BOUNDARY_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ALL_PATH_OPERATION_BOUNDARY_BLOCKER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROTOCOL_REAL_SITE_BLOCKER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROTOCOL_PREFIX_RECORDER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROTOCOL_FAILURE_TERMINAL_AUTHORITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROTOCOL_FAILURE_BUNDLE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_PROTOCOL_FAILURE_VERIFICATION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_SOURCE_ARCHIVE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_PATH_GAP_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COVERAGE_REPORT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_SOURCE_BLOCKER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COVERAGE_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_ZERO_EXECUTION_WINDOW_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_ZERO_VALUE_PROOF_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_RESIDUAL_GAP_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_ZERO_VALUE_CLOSURE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_ZERO_VALUE_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_QUERY_OWNER_WINDOW_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_QUERY_OWNER_RESOLUTION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_QUERY_OWNER_ENVELOPE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_QUERY_OWNER_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_LIFECYCLE_WINDOW_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_LIFECYCLE_RESOLUTION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_LIFECYCLE_ENVELOPE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_LIFECYCLE_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COMMON_SHARED_WINDOW_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COMMON_SHARED_RESOLUTION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COMMON_SHARED_ENVELOPE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_CERTIFIED_COMMON_SHARED_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_RUNTIME_PREPARATION_V2_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_WORKER_OUTPUT_V2_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_MEASUREMENT_WINDOW_V2_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_SHARED_RESOLUTION_V2_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_SHARED_ENVELOPE_V2_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_ACCOUNTED_SHARED_REPLAY_V2_DOMAIN,
        ids.CONSTRUCTION_K7_DIRECT_FALLBACK_EXACT_INFEASIBILITY_READINESS_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_PREEXECUTION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_CURRENT_IDENTITY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_CARDINALITY_SOURCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_TRANSITION_TRACE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_PATH_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_ACQUISITION_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CANONICAL_INFEASIBLE_FALLBACK_SUPPORT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_RETAINED_V1_INVENTORY_CONTEXT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_LEGACY_SHARED_AGGREGATE_CLAIM_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_LEGACY_OWNER_EVENT_CANDIDATE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_LEGACY_RECONCILIATION_CLAIM_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_RETAINED_V1_FORMAL_BLOCKER_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_RETAINED_V1_EVIDENCE_INVENTORY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_ABSTRACT_PASS_RETAINED_V1_INVENTORY_REPLAY_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CONDITIONAL_TERMINAL_NORMALIZATION_PROFILE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CONDITIONAL_TERMINAL_NORMALIZATION_RULE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CONDITIONAL_TERMINAL_NORMALIZATION_EVIDENCE_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CONDITIONAL_TERMINAL_NORMALIZATION_RESULT_V1_DOMAIN,
        ids.CONSTRUCTION_K7_CONDITIONAL_TERMINAL_NORMALIZATION_REPLAY_V1_DOMAIN,
    )
    assert len(domains) == len(set(domains)) == 118
    assert set(domains) <= ids.PHASE3E_DOMAIN_TAGS
    payload = {"schema": "same-k7-accounting-successor-payload"}
    assert len({ids.content_id(domain, payload) for domain in domains}) == len(domains)
