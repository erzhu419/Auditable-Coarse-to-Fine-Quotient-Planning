...
def test_phase3a_bundle_uses_its_complete_profile_contract(tmp_path: Path) -> None:
    documents = {
        path: ([{"event": "complete"}] if path.endswith(".jsonl") else {"path": path})
...
def test_phase3d_bundle_uses_its_complete_profile_contract(tmp_path: Path) -> None:
    documents = {
        path: ([{"event": "complete"}] if path.endswith(".jsonl") else {"path": path})
    ...
    assert manifest["required_paths"] == sorted(PHASE3D_REQUIRED_PATHS)
