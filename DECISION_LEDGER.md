...
| V0-RISK-004 | 2026-07-20 | General local value-risk search | OPEN | The contract-0.8 worker proves both value and risk for the registered fixture, but selects each reopened state's deterministic action by the frozen minimum-risk/maximum-reward order. It does not yet jointly enumerate Pareto action combinations across a general multi-cell frontier, so a future instance could contain a certifiable trade-off that this solver conservatively misses. | No effect on V0-029's exhaustive two-cell minimality result or soundness; before a general local-recovery completeness claim, add joint Pareto composition, charged caps, and synthetic trade-off regressions. | `LocalRepairSolver`; value-risk frontier composition; candidate accounting; general-domain recovery tests. |
| V0-RISK-005 | 2026-07-20 | General DirectBad causal minimality | OPEN | `DirectBad` is now restricted to Bellman residuals in the actually failed value/risk channel, and raw successor TV alone is insufficient. It does not yet propagate certificate slack or an explicit active worst-case derivation, so a large proof DAG may authorize a relevant residual that has no material contribution to the root deficit. | No effect on the registered V0-029 frontier, which is independently exhaustive and uniquely minimal. A stronger general locality claim requires slack-aware causal cones and no-contribution counterexamples. | Failed-proof DAG; frontier extractor; slack/worst-case witnesses; locality verifier. |
| V0-RISK-006 | 2026-07-20 | Boundary capability minimization | OPEN | The isolated worker receives no ground identities, payloads, RAPM, kernel, coverage, J0, or accounting, but its redacted boundary still contains every selected-policy abstract realization row needed by the current exact recursion rather than a proven minimum set of handoff scalar bounds. | Current V0-029 three-input isolation claim remains valid. Before claiming a minimal general capability surface, derive and verify a smaller sufficient boundary representation. | `RedactedBoundaryView`; isolated runtime schema; information-flow tests; capability-minimality proof. |
