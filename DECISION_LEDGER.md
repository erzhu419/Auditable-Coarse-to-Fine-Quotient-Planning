...
| V0-075 | 2026-07-29 | Fresh total-lift parallel confirmatory campaign | PRE-ANCHOR CONSTRUCTION / TARGET LOCKED / GATE NOT RUN | Proposed contract `1.40.0` and profile `v075_fresh_total_lift_parallel_confirmatory_campaign_v0` define a new authority family rather than a V0-072 retry.  Three replicate contexts retain the public K7/W7/K7-minus-two structures.  The exact laws proposed during construction were publicly exposed and are now permanently `CONSTRUCTION_FIXTURE_ONLY`; production uses a law-free public dependency graph and may bind a separately held private environment only through a high-entropy salted opaque commitment.  The one-way authority chain is historical-exclusion registry plus public target-family generation plus a semantically verified compact source-proposal archive plus that opaque commitment, then a new execution manifest, final preregistration, first qualifying `origin/main` anchor, anchor-derived tape namespace and immutable 3×5 occurrence plan.  V0-075 now has a profile-neutral exact H2 core, law-free signed observation graph, private in-memory reveal/observer boundary, source-only historical replay controller, strict manifest/preregistration and independent Git-anchor verifier, plus canonical-bytes-only process input, precreated private journals, CAS chunks and strict ordinal merge. | Both V0-072 failure records and all historical target observations, models, plans, certificates, journals, results, caches and retry authority are forbidden scientific inputs.  Verified source-only evidence and pure planning/exact-arithmetic code may be reused, but source work remains a charged offline lane: the compact recipe omits the 21 native values.  A source-only exact replay path now bypasses the historical manifest's target-module closure while independently matching the frozen recipe, yet the expensive replay itself remains `NOT_RUN`.  The independent remote verifier also deliberately leaves every production authority-role semantic verifier unavailable, so no final manifest, anchor or production-open authority can yet be minted.  Until production occurrence workers, total lift, reconciliation, endpoint, semantic role verifiers, tracked artifacts and the first qualifying remote anchor all pass, target execution remains forbidden and sample-efficiency, official, scalar, economics and counter-completeness Gates remain false/null/`NOT_RUN`. | `specs/FRESH_TOTAL_LIFT_PARALLEL_CONFIRMATORY_CAMPAIGN.md`; law-free public authority/graph semantics; construction-only exposed-law fixture; source archive/work/prior authorities and source-only replay controller; `h2_graph_transition_engine_v1.py`; private observer boundary; CAS/journal transport; manifest/preregistration and independent remote-anchor verifier; future adaptive/direct workers, total-lift terminal authority, reconciliation, endpoint, attempt journal and complete-bundle verifier. |
