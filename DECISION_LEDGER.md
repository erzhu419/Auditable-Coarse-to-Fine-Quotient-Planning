...
- **2026-08-08 — contracts 2.0.50--2.0.57 / V0-110B-2E-39..46:** Froze the H1 two-role recipe, branch-aware structural output control, honest current-identity candidate, path-free/source-owned exact fallback segment, construction adapter/topology, exact V6 accounting replay and the fresh-exec current-access authority. The resulting current-access authority is predecision and one-shot, but no formal shared operand, V7 decision or execution authority was issued.
- **2026-08-08 — contracts 2.0.58-A--C / V0-110B-2E-47A--C:** Replaced the nine non-callable sentinels with a serialized issuer-owned construction owner and added explicit registered-template catalogue candidates for common/read/stage/mount/memory/launch work. Independent audit demoted both downstream tables: the 10/90 output table is not a native lifecycle authority, its provisional ceilings and placeholder 202/182/8 documents are not formal V6 bounds, and the resource table does not prove production multiplicities, failure prefixes, physical identities or owner-compatible mount intervals. The owner manifest is only a structural site declaration, not source-byte/AST/loaded-symbol authority; owner errors issue local failure kinds rather than attempt terminals. Output, memory, launch and inode/OFD aliases use candidate-specific schemas/domains/IDs, deprecated authority APIs fail closed, and embedded accounting/terminal payloads use nonformal `.width_witness.v1` schemas. COPY is only a distinct structural target slot; BIND remains distinct until native inode/OFD evidence; memory preexecution caps/PID membership are separated from post-run peak receipt; BUSINESS-launch ambiguity is explicitly missing. The owner preserves known overruns, exact-once reap observations and a primary/secondary cleanup cause chain, while production activation, native evidence, persistent consumption and ambiguity resolution remain blocked. No numeric operand, formal V7 upper/decision, execution, accounting artifact, terminal or Gate result is issued. Official execution, scalar/break-even, workload economics, Counter Completeness and sample efficiency remain locked or `NOT_RUN`.
- **2026-08-08 — contract 2.0.58-D / V0-110B-2E-47D:** Added a content-addressed but externally unanchored H1 lifecycle candidate whose single 62-transition tuple drives candidate-prefix replay and 144 derived first-failure prefixes relative to its declared edges. It uses a candidate-specific module/profile/schema/domain/ID; every deprecated authority accessor and verifier fails closed. It lists memory-first admission, both child-launch ambiguity edges, declared stage/read/mount admissions, reap, same-OFD peak settlement, cleanup order and output finalization; complete module bytes, normalized AST and five defining source spans are bound. This is not production source authority or a completeness proof: a fresh import can self-mint a changed candidate, the live broker/worker/business runtime does not dispatch it, common multiplicity and cap-owner semantic identity are unbound, output-DAG role leaves are not joined to readback, and post-first-failure cleanup continuation is absent. First failure is provisional and issues neither attempt closure nor terminal classification. It issues no numeric operand, V7 upper/decision, execution, accounting artifact, terminal or Gate result; all locks remain unchanged.
- **2026-08-08 — contract 2.0.59-A / V0-110B-2E-48A:** Added caller-pinned local-Git provenance for the Contract-2.0.58-D lifecycle migration seed through a non-circular K -> C record. K binds a canonical snapshot and twelve explicit non-transitive component blobs; C must add exactly one `100644` preregistration. The independent verifier disables Git replace objects, rejects shallow/non-SHA-1 histories, checks object types, exact tree diff/current modes, and independently derives 62 transitions, 143 failure edges and 144 branches plus their content IDs without importing the candidate module. This does not eliminate caller self-selection: the expected anchor ID still comes from the caller, no earlier activation binds it, the component set is not a transitive semantic closure, embedded source-manifest/topology/output-DAG IDs are not independently rederived, and the instantaneous worktree check is neither TOCTOU-safe nor a loaded-byte proof. Accordingly `source_authority_present=false`, `fresh_import_self_mint_prevented=false`, `usable_as_execution_source=false` and `remote_published=false`. Live dispatch, cleanup continuation, owner V3, output-leaf join, numeric operands, V7 routing, accounting and every Gate remain locked.
- **2026-08-08 — contract 2.0.59-B / V0-110B-2E-48B:** Froze a construction-only durable owner for the nine H1 shared-resource paths and one route-attempt-wide rejection gate. The owner uses transaction-bound exact SUM/MAX profiles, reserve-before-effect lifecycle records, a complete cell/evidence/settlement/receipt/event/snapshot recovery frontier, inode-pinned append-only cursors and producer/replay-equivalent semantic validation. The gate uses gate-first cap rejection, thread/context-bound admission leases, prevalidated ACK operands and exact owner-pair acknowledgement. Exact pending reservations can converge after a later transaction closes the attempt; an ambiguous pending start can only complete through conservative settlement without effect re-execution; durable overrun evidence immediately poisons new work. Missing admission, pair and ACK prefixes, external rejection ownership, concurrent open/replay, rollback, reentry, oversize, re-signed semantic mismatch and partial-initialization consumption all fail closed or recover only by their exact registered continuation. This remains caller-asserted construction arithmetic: production source/native authority, credential isolation, live dispatch, cleanup continuation, output/read fixed point, numeric V7 operands/decision, formal `CounterRecord -> WorkVector -> ComparisonVector`, terminal/campaign closure and the complete-bundle verifier remain absent. `official_execution_allowed=false`; scalar/break-even, economics, Counter Completeness and sample efficiency remain null, locked or `NOT_RUN`.
- **2026-08-08 — contract 2.0.59-C / V0-110B-2E-48C:** Bound the caller-pinned 62-site lifecycle snapshot to an ordered construction dispatcher and the durable Owner-V3 settlement chain. The typed registry freezes `5/41/2/2/12` immediate-unit, immediate-magnitude, admission-only deferred-origin, deferred-completion and no-charge handlers; 48 reservation uppers are prebound and callers can invoke only the next site. Memory/output origins reserve only and issue no native-start evidence; their completions reuse and settle those reservations. Exact trace replay checks schemas, content IDs, state/outcome semantics, journal sequence/head, attempt-gate commit/ACK state and complete Owner record-set coverage; callback failure/invalid value, cap rejection, unclipped overrun, reentry, callable drift, coherent re-signing, identity crossing, out-of-band append and external-transaction gate closure fail closed. Pre-admission validation may issue no event and remain retryable. Owner records are durable, but dispatch events remain in-memory and all values remain construction assertions. Production source/live/native authority, complete cleanup continuation, output/read joins, current-access bridge, numeric V7 routing, formal vectors, terminal/campaign closure and all official/economics/sample Gates remain absent, false, null or `NOT_RUN`.
- **2026-08-08 — contract 2.0.59-D / V0-110B-2E-48D:** Replayed the unchanged anchored `62/143/144` lifecycle identities, added the ten emitted mount-open overrun protocol-abort branches omitted by the old edge grammar, and froze a `154`-row registered analysis universe: `143` declared candidate failures (two dispatcher-unreachable), ten dispatcher-emitted supplemental aborts and one success. Admission-only memory/output reservations never fabricate native objects; plans preserve immutable primary failure, ordered secondary causes, BUSINESS-before-WORKER reap, conservative reservation settlement and LIFO mount closure. Added a synchronous append-only Owner-prefix verifier that preserves the original exact-tail verifier but emits no tail-bound attestation, plus an exact-source-byte, Python-minor-tolerant structural cross-check whose artifact IDs remain runtime-specific. Bound all ten registered output contexts, ninety leaves, sixteen presence sets and eight readback sites one-to-one, with typed absent-role skips; the linear dispatcher cannot yet execute those skips. Cleanup remains `NOT_RUN`; no-event post-admission recovery, terminal-context/commit/readback authority, cleanup-only gate lease, crash-safe step journal or FQ11 cleanup cost leaf exists. Live/current-access/fixed-point, V7, formal accounting, terminal/campaign and complete-bundle authority remain open; all official/scalar/economics/Counter-Completeness/sample Gates stay false, null or `NOT_RUN`.
- **2026-08-08 — contract 2.0.59-E-A / V0-110B-2E-48E-A:** Added a non-relabeling Owner-V4 wrapper whose exact next-record payload is durable before pending cursor `P`, plus deterministic recovery across WAL/P/journal/C/unlink and root-marker crash frontiers. Activation publishes its binding only after a complete seven-record zero-valued control intent; durable intent/namespace evidence cannot be downgraded, public retry and concurrent initialization converge, and the sole pre-intent empty namespace is safely rolled back under Owner lock even after a later gate rejection. Added a runtime-specific semantic closure covering the actual attestor/V4/index/dispatcher path, registered closure-cell originals, deep mutable globals, project behavior descriptors and the registered generator-context-manager protocol, while explicitly excluding hostile interpreter/stdlib completeness and cross-process authority. Tail issuance verifies the dispatch prefix and requires equal gate/Owner observations around the record index, then binds the caller-pinned ordered tail and exact extension in a content-addressed artifact. Later appends invalidate exact-current replay and require a predecessor-linked successor; there is no atomic future-consumer lease or exact-current-use authority. Admitted no-event recovery, cleanup-only execution, terminal output/readback/current-access authority, V7, formal accounting, terminal/campaign and complete-bundle authority remain absent; all official/scalar/economics/Counter-Completeness/sample Gates remain false, null or `NOT_RUN`.
- **2026-08-08 — contract 2.0.59-E-B / V0-110B-2E-48E-B:** Added an independent attempt-wide phase owner with the sole monotonic chain `NORMAL -> CLEANUP_INTENT_DURABLE -> CLEANUP_ONLY`, separate from attempt cap rejection. Its specification binds attempt/provenance/gate/program/registry/cleanup-analysis identity; its transition binds the failing transaction, Owner-V4 runtime/WAL, dispatch trace/profile, live semantic closure, exact observed tail, cleanup pass and immutable primary failure under ordered `PHASE -> GATE -> OWNER` locks. Allocation, root lock, phase lock and cursor inodes are bound; genesis is atomically published; intent, root seal and commit are identical hard links; replay repairs only a strict prefix of the uniquely expected next cursor frame and restores phase-local rollback from the root seal. Normal, transition-only and cleanup-phase leases are exclusive, PID/thread-bound and nonserializable; immutable publication conservatively consumes NORMAL authority before the first write, and subprocess exits at all four durable boundaries converge without context cleanup. Fork-in-lease finalization is fail-closed: inherited children clear only copied contexts and descriptor references without `LOCK_UN`, only the creating PID/thread releases locks, and a real child-unwind test keeps a contender blocked behind the live parent. This is a phase primitive, not execution integration: historical dispatcher/Owner APIs reacquire the retained gate, historical cap rejection cannot activate V4 or mint post-rejection exact-prefix evidence, and cleanup execution is absent. Durable normal-site intent/result, no-event recovery, cleanup envelope/journal, conditional output skip/readback, current access, V7, formal accounting, terminal/campaign and complete-bundle authority remain open; all official/scalar/economics/Counter-Completeness/sample Gates remain false, null or `NOT_RUN`.
- **2026-08-08 — contract 2.0.59-E-C-A / V0-110B-2E-48E-C-A:** Added a phase-aware but explicitly pretransition executor for anchored lifecycle ordinals `1..40`. One retained `PHASE -> GATE -> NORMAL JOURNAL -> OWNER -> native` lease avoids historical same-gate reentry. Durable intent precedes admission, native-start precedes callback, callback result precedes settlement, and the event binds the exact Owner record interval; deterministic in-process recovery closes a missing result after native start without callback replay, while registered cap-rejection fault injection converges the exact gate commit, Owner admission/pair, ACK and zero-callback event. These are construction mechanics, not yet broad crash-recovery authority: real process-exit coverage across all boundaries remains absent and the corresponding flags stay false. The private journal binds allocation/lock/cursor inodes, immutable record seals and an inode-bound high-water frontier; only adjacent append crash states can repair, while rollback below an intact high-water frontier, token/state substitution, semantic allocation re-signing and ordinal drift fail closed. This assumes a trusted bound-root namespace; any actor able to coherently mutate or relink it requires an external monotonic witness. All used upstream symbols are import-registered, executed through captured objects and checked for live namespace or registered mutable-constant drift. Callback continuation is PID/thread-bound; registered namespace/view/lease/input mutation fails closed, but same-process Python is not a hostile-code isolation boundary and production still requires fresh-exec roles. The broad completion flags remain false: success stops before ordinal 41, failure/rejection leaves the phase `NORMAL` with a poisoned prefix, and no cleanup transition/pass/executor exists. Cleanup/skip/readback, output fixed point, current access, V7, formal accounting, terminal/campaign and complete-bundle authority remain open; official/scalar/economics/Counter-Completeness/sample states remain false, null or `NOT_RUN`.
- **2026-08-09 — contract 2.0.59-E-C-B / V0-110B-2E-48E-C-B:** Pre-admitted before ordinal 1 an exact structural cleanup envelope for normal-prefix ordinals `1..40`, retaining 112 registered branches, 111 dispatcher-reachable branches and one explicit unreachable negative control. Integrated normal execution validates the envelope before callback entry and binds the first durable failure or cap rejection, exact Owner tail, retained gate and selected cleanup pass into a tagged V2 `NORMAL -> CLEANUP_INTENT_DURABLE -> CLEANUP_ONLY` transition without synthesizing a legacy trace. Successor replay delegates actual historical V1 transitions to the unchanged V1 parser and independently rebinds V2 envelope/branch/gate plus identical intent/seal/commit; missing terminal commit, coherent claim rehash, cross-artifact binding and failed nested acquisition are rejected. The V2 cleanup-only lease is only a phase/gate barrier. It retains no cleanup budget, PIDFD/OFD/native capability, Owner continuation, cleanup executor, production output, current-access, V7, formal vector, terminal/campaign or complete-bundle authority. All official/scalar/economics/Counter-Completeness/sample states remain false, null or `NOT_RUN`.
- **2026-08-09 — contract 2.0.59-E-C-C / V0-110B-2E-48E-C-C:** Added a separate append-only Owner cleanup sidecar for the memory/output admission-only deferred reservations. One canonical phase-root allocation binds the exact V3/V4 cutoff, V2 transition, envelope, cleanup pass/action and reservation; alternate roots cannot duplicate the spend. The only operation is `CONSERVATIVE_RELEASE_WITHOUT_NATIVE_START`: it charges the full upper under the original reducer, records typed-null native observation and releases outstanding exposure exactly once without changing V3/V4, reading memory or finalizing output. A read-only stable-V4 preflight rejects all repairable or ambiguous Owner frontiers before mutation; allocation/release seals, inode binding, validate-before-truncate cursor repair and descriptor-failure cleanup close the registered recovery paths. This is not native or arbitrary cleanup execution and issues no output-owner close/readback, current-access, V7, formal vectors, attempt/terminal/campaign, production or official authority. All scalar/economics/Counter-Completeness/sample states remain null, locked or `NOT_RUN`.
- **2026-08-09 — contract 2.0.59-E-C-NATIVE-A / V0-110B-2E-48E-C-NATIVE-A:** Added an exact pre-ordinal-1 native-resource receipt journal for ten mount OFD slots and two child PIDFD slots. Both spec freeze and allocation publication retain the normal-journal lock across genesis validation, so ordinal 1 cannot cross either declaration gate. The immutable `START -> CALLBACK_RESULT -> exact normal-event-bound receipt/absence` stream forbids callback replay after start, derives all slot semantics from the sealed spec, rejects later evidence at an earlier cutoff and makes cutoff terminal. Cursor/high-water/seal/inode replay and same-broker crash convergence are fail closed; a dead broker cannot be replaced and a foreign process cannot publish recovery seals or completion. Raw descriptors are never serialized, opaque identities are not kernel credentials, and the receipt is construction evidence only. Native cleanup, V2-transition integration, current access, production execution, formal counters/vectors, V7 and all official/economics/sample Gates remain absent, false, null or `NOT_RUN`.
