...
- **2026-08-01 — contract 1.94.0:** Added a real bounded read-only K7 OS-supervisor host probe and a mandatory preopened delegated-parent-FD admission boundary. The current host closes before launch as typed noncertificate because no delegated parent is supplied. A supplied directory FD remains `PREFLIGHT_ONLY` until a real exclusive cgroup lease, atomic cgroup child placement, pidfd lifecycle and post-reap peak verifier exist; no nine-path semantic or formal-vector lock moves.
