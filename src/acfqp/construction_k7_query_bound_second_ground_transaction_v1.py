"""Execute the final registered query-local recovery transaction.

The transaction-2 request is reverified before a new observer namespace is
created.  Each selected semantic row receives one fresh support-discovery
batch and exactly the requested final validation increment.  The controller
then closes and reconciles.  This slice records signed evidence only; model
compilation and replanning are the next pure-abstract step.
"""
...
from acfqp import construction_k7_query_bound_second_recovery_request_v1 as request_v1
...
PROPOSED_CONTRACT_VERSION = "2.0.91"
PROFILE_KEY = "construction_k7_query_bound_second_ground_transaction_v1"
ENVIRONMENT_MARKER = "real-reusable-build-epoch"

NAMESPACE_DOMAIN = (
    CONSTRUCTION_K7_QUERY_BOUND_TRANSACTION_2_NAMESPACE_BINDING_V1_DOMAIN
)
ROW_EXECUTION_DOMAIN = (
    CONSTRUCTION_K7_QUERY_BOUND_TRANSACTION_2_ROW_EXECUTION_V1_DOMAIN
)
TRANSACTION_DOMAIN = (
    CONSTRUCTION_K7_QUERY_BOUND_TRANSACTION_2_GROUND_TRANSACTION_V1_DOMAIN
...
if len(LOCAL_DOMAINS) != 3 or not LOCAL_DOMAINS <= PHASE3E_DOMAIN_TAGS:  # pragma: no cover
    raise RuntimeError("query-bound transaction-2 execution domains are not central")
...
class ConstructionK7QueryBoundSecondGroundTransactionV1Error(ValueError):
    """The final local request, namespace, or signed work inventory changed."""


def _fail(message: str) -> NoReturn:
    raise ConstructionK7QueryBoundSecondGroundTransactionV1Error(message)
...
def _cid(value: Any, label: str) -> str:
    try:
        ...
        raise ConstructionK7QueryBoundSecondGroundTransactionV1Error(
            f"{label} must be one exact content ID"
        ) from error


@dataclass(frozen=True, slots=True)
class SecondQueryBoundNamespaceBindingV1:
    ...
    def __post_init__(self, _issuer: object) -> None:
        if _issuer is not _NAMESPACE_ISSUER:
            _fail("transaction-2 namespace binding is caller-minted")
        for value, label in (
            (self.predecessor_replanning_id, "predecessor replanning"),
            (self.recovery_request_id, "transaction-2 request"),
            (self.logical_occurrence_id, "logical occurrence"),
            (self.target_tape_namespace_id, "transaction-2 namespace"),
            (self.native_occurrence_id, "transaction-2 occurrence"),
        ...
        if self.arm != worker_v1.V075WorkerArmV1.NO_PRIOR.value:
            _fail("transaction-2 namespace arm changed")
        object.__setattr__(
            self,
            "_binding_id",
    ...
    def _payload(self) -> dict[str, Any]:
        return {
            "schema": "acfqp.construction_k7_query_bound_transaction_2_namespace_binding.v1",
            ...
            "namespace_differs_from_transaction_1": True,
    ...
    def binding_id(self) -> str:
        ...
        if current != self._binding_id:
            _fail("transaction-2 namespace binding changed after issuance")
        return current
...
@dataclass(frozen=True, slots=True)
class SecondQueryBoundRowExecutionV1:
    _issuer: InitVar[object]
    validation_request_id: str
    source_numerical_row_id: str
    ...
    def __post_init__(self, _issuer: object) -> None:
        if _issuer is not _ROW_ISSUER:
            _fail("transaction-2 row execution is caller-minted")
        for value, label in (
            (self.validation_request_id, "transaction-2 validation request"),
            (self.source_numerical_row_id, "transaction-2 numerical row"),
            (self.semantic_row_binding_id, "transaction-2 row binding"),
            (self.discovery_intent_id, "transaction-2 discovery intent"),
            (self.discovery_batch_id, "transaction-2 discovery batch"),
            (self.discovery_append_receipt_id, "transaction-2 discovery receipt"),
            (self.support_freeze_id, "transaction-2 support freeze"),
            (self.validation_intent_id, "transaction-2 validation intent"),
            (self.validation_batch_id, "transaction-2 validation batch"),
            (self.validation_append_receipt_id, "transaction-2 validation receipt"),
        ):
            _cid(value, label)
        ...
        if (
            ...
            or self.requested_validation_draw_count
            != caps.promotion_validation_draws_per_round
            or self.target_validation_draw_count != 12_288
        ):
            _fail("transaction-2 row execution draw counts changed")
        object.__setattr__(
            self,
            "_execution_id",
    ...
    def _payload(self) -> dict[str, Any]:
        return {
            "schema": "acfqp.construction_k7_query_bound_transaction_2_row_execution.v1",
            "schema_version": SCHEMA_VERSION,
            "proposed_contract_version": PROPOSED_CONTRACT_VERSION,
            "profile_key": PROFILE_KEY,
            "validation_request_id": self.validation_request_id,
            "source_numerical_row_id": self.source_numerical_row_id,
            ...
            "target_validation_draw_count": self.target_validation_draw_count,
    ...
    def execution_id(self) -> str:
        ...
        if current != self._execution_id:
            _fail("transaction-2 row execution changed after issuance")
        return current
...
@dataclass(frozen=True, slots=True)
class SecondQueryBoundGroundTransactionV1:
    _issuer: InitVar[object]
    predecessor: replanning_v1.QueryBoundOverlayReplanningV1 = field(repr=False)
    request: request_v1.SecondQueryBoundRecoveryRequestV1 = field(repr=False)
    namespace_binding: SecondQueryBoundNamespaceBindingV1
    ...
    row_executions: tuple[SecondQueryBoundRowExecutionV1, ...]
    ...
    def __post_init__(self, _issuer: object) -> None:
        if (
            ...
            is not replanning_v1.QueryBoundOverlayReplanningV1
            or type(self.request)
            is not request_v1.SecondQueryBoundRecoveryRequestV1
            or type(self.namespace_binding)
            is not SecondQueryBoundNamespaceBindingV1
            ...
            or any(
                type(item) is not SecondQueryBoundRowExecutionV1
                for item in self.row_executions
            )
        ):
            _fail("transaction-2 ground transaction is caller-minted")
        replanning_v1.verify_query_bound_overlay_replanning_v1(self.predecessor)
        request_v1.verify_second_query_bound_recovery_request_v1(
            self.request,
        ...
        if (
            self.namespace_binding.predecessor_replanning_id
            != self.predecessor.result_id
            or self.namespace_binding.recovery_request_id != self.request.request_id
            ...
            or self.namespace_binding.target_tape_namespace_id
            == self.predecessor.transaction.namespace_binding.target_tape_namespace_id
            or tuple(item.validation_request_id for item in self.row_executions)
            != tuple(item.request_id for item in requested)
            or tuple(item.source_numerical_row_id for item in self.row_executions)
            ...
            _fail("transaction-2 identity or work inventory crossed")
        ...
        if replayed != self.observer_closure.reconciliation:
            _fail("transaction-2 observer closure differs from exact replay")
        appends = {item.batch.batch_id: item for item in self.observer_closure.appends}
        if len(appends) != len(self.observer_closure.appends):
            _fail("transaction-2 observer closure duplicated a batch")
        for execution in self.row_executions:
            discovery = appends.get(execution.discovery_batch_id)
            validation = appends.get(execution.validation_batch_id)
            if (
                discovery is None
                or validation is None
                or discovery.receipt.receipt_id
                != execution.discovery_append_receipt_id
                or validation.receipt.receipt_id
                != execution.validation_append_receipt_id
                or discovery.batch.request.stream_identity.lane.value != "DISCOVERY"
                or validation.batch.request.stream_identity.lane.value != "VALIDATION"
                or discovery.batch.request.stream_identity.row_binding_id
                != execution.semantic_row_binding_id
                or validation.batch.request.stream_identity.row_binding_id
                != execution.semantic_row_binding_id
                or discovery.batch.request.accepted_draw_count
                != execution.support_discovery_draw_count
                or validation.batch.request.accepted_draw_count
                != execution.requested_validation_draw_count
            ):
                _fail("transaction-2 row execution crossed its signed appends")
        object.__setattr__(
            self,
            "_transaction_id",
    ...
    def _payload(self) -> dict[str, Any]:
        return {
            "schema": "acfqp.construction_k7_query_bound_transaction_2_ground_transaction.v1",
            "schema_version": SCHEMA_VERSION,
            "proposed_contract_version": PROPOSED_CONTRACT_VERSION,
            "profile_key": PROFILE_KEY,
            "predecessor_replanning_id": self.predecessor.result_id,
            "transaction_2_recovery_request_id": self.request.request_id,
            "logical_occurrence_id": self.request.logical_occurrence_id,
            "source_numerical_model_id": self.request.source_model_id,
            ...
            "observer_control_closure_id": (
                self.observer_closure.control_closure.control_closure_id
            ),
            "observer_reconciliation_id": (
                self.observer_closure.reconciliation.reconciliation_id
            ),
            ...
            "namespace_distinct_from_transaction_1": True,
            "only_requested_rows_executed": True,
            "observer_closed_and_exactly_reconciled": True,
            "final_local_transaction_executed": True,
            "local_transaction_budget_exhausted_after_this_attempt": True,
            "immutable_overlay_compiled": False,
            "post_transaction_replanning_performed": False,
            "next_required_action": "COMPILE_FINAL_LOCAL_OVERLAY_AND_REPLAN",
            "construction_only": True,
    ...
    def transaction_id(self) -> str:
        ...
        if current != self._transaction_id:
            _fail("transaction-2 ground transaction changed after issuance")
        return current
    ...
    def to_document(self) -> dict[str, Any]:
        return {
            **self._payload(),
            ...
            "query_bound_transaction_2_ground_transaction_id": self.transaction_id,
        }
...
def _row_binding(
    ...
    state = graph_v1.V075SymbolicGraphStateV1(model.context, row.source_ranks, False)
    ...
    binding = graph_v1.observation_row_binding_v1(model.context, catalogue, row.action)
    if binding.row_binding_id != row.row_binding_id:
        _fail("transaction-2 numerical row changed its semantic binding")
    return binding


def execute_second_query_bound_ground_transaction_v1(
    *,
    predecessor: replanning_v1.QueryBoundOverlayReplanningV1,
    request: request_v1.SecondQueryBoundRecoveryRequestV1,
) -> SecondQueryBoundGroundTransactionV1:
    predecessor = replanning_v1.verify_query_bound_overlay_replanning_v1(predecessor)
    request = request_v1.verify_second_query_bound_recovery_request_v1(
        request,
        predecessor=predecessor,
    )
    model = predecessor.successor_model
    if model.model_id != request.source_model_id:
        _fail("transaction-2 source model crossed its request")
    prepared = fixture_v1.prepare_v075_k7_construction_environment_v1(
        environment_marker=ENVIRONMENT_MARKER,
        identity_marker=request.request_id,
    )
    contexts = tuple(
        item
    ...
    if len(contexts) != 1 or contexts[0] != model.context:
        _fail("transaction-2 namespace does not share the reusable context")
    context = contexts[0]
    ...
    for item in request.requested_rows:
        ...
        if row is None or row.row_binding_id != item.semantic_row_binding_id:
            _fail("transaction-2 requested row is absent from source model")
        ...
        executions.append(
            SecondQueryBoundRowExecutionV1(
                ...
                item.next_registered_checkpoint,
            )
        )
    closure = controller.close_and_reconcile_v2()
    return SecondQueryBoundGroundTransactionV1(
        _TRANSACTION_ISSUER,
        predecessor,
        request,
        namespace_binding,
...
def verify_second_query_bound_ground_transaction_v1(
    claimed: SecondQueryBoundGroundTransactionV1,
) -> SecondQueryBoundGroundTransactionV1:
    if type(claimed) is not SecondQueryBoundGroundTransactionV1:
        _fail("transaction-2 ground transaction has a foreign type")
